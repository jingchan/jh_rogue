...
public abstract class JGEngine extends MIDlet implements JGEngineInterface {
	...
	public void drawTile(int xi,int yi,int tileid) {
		...
		if (img==null||el.images_transp.containsKey(tileid_obj)) {
			...
			if (bg_image==null) {
				...
				bgg.drawRegion(((MIDPImage)el.getImageQuick(bg_image.imgname)).img,
	...
	* passing a null means ignore that argument. */
	...
	Font getFont(JGFont jgfont) {
		...
		if (fontsize <= 8) return smallfont;
		if (fontsize <= 16) return mediumfont;
		return largefont;
	}
	...
	boolean pf_relative) {
		...
		MIDPImage img = (MIDPImage)el.getImageQuick(imgname);
		if (img!=null)
...
//	} }
	...
	* than 1 sample + 1 midi file at once.  So there are only 2 channels,
	* chosen by file type of the audio file. */
	// see also:
	...
	public void playAudio(String channel,String clipid,boolean loop) {
		if (!players.containsKey(clipid) && loop)
		...
		try {
			if (cur_player!=null) {
				...
				if (cur_player.getState()==Player.STARTED) return;
				// player stopped -> free audio device
				// (most devices can play only 1 sample at a time)
				cur_player.deallocate();
				cur_player=null;
			}
