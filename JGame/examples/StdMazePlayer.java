		...
		if (!isXAligned(rawspeed/2.0) || !isYAligned(rawspeed/2.0)) {
			...
			if (go_up) {
				if (!and(eng.getTileCid(cen,0,-1),block_mask)) {
					setDir(0,-1);
					if (gfx_prefix!=null) setGraphic(gfx_prefix+"u");
				}
				moving=1;
			}
			if (go_down) {
				if (!and(eng.getTileCid(cen,0,1),block_mask)) {
					setDir(0,1);
					if (gfx_prefix!=null) setGraphic(gfx_prefix+"d");
				}
				moving=1;
			}
			if (go_left) {
				if (!and(eng.getTileCid(cen,-1,0),block_mask)) {
					setDir(-1,0);
					if (gfx_prefix!=null) setGraphic(gfx_prefix+"l");
				}
				moving=1;
			}
			if (go_right) {
				if (!and(eng.getTileCid(cen,1,0),block_mask)) {
					setDir(1,0);
					if (gfx_prefix!=null) setGraphic(gfx_prefix+"r");
				}
				moving=1;
			}
